...
We therefore address this missing-reference issue with
...
scores with image-conditioned prototypes. The added module changes the
question from using local response strength as final evidence to asking how
abnormal a patch is relative to a normal reference estimated from the current
image.
...
\begin{itemize}
  ...
  wavelet-map fusion serves as a negative control, GlobalRef tests a fixed/global
  reference, semantic-only prototype adaptation is a strong
  baseline, and boundary-aware reliability plus conservative calibration gives
  the best controlled row on MVTec AD and VisA.
\end{itemize}
