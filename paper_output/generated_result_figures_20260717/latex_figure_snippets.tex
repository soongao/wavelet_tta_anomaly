...
\caption{Main result summary across five anomaly detection datasets. (a) Pixel-level AUPRO comparison between the AnomalyCLIP baseline and Ours. Values on the right report absolute point gains. (b) Ours gains over the baseline across pixel- and image-level metrics.}
...
\caption{Core component ablation on MVTec and VisA. Each panel reports one evaluation metric for the fixed baseline, direct wavelet fusion without prototype adaptation, semantic-only prototype adaptation, Ours w/o conservative update, and Ours.}
...
\caption{Dataset-level gains of Ours over the AnomalyCLIP baseline on the two headline metrics.}
