...
最终投稿版必须补齐实现细节，使 WPTA 可复现。需要报告的项目包括：CLIP backbone 与 feature layers、输入分辨率、Haar DWT 的 feature-grid 尺度、`HF` 与 `LF_edge` 的归一化方式、`rho(.)` 的具体形式、evidence patch 的 top-k 或阈值、`gamma`、`alpha`、`beta`、confidence gate 规则、anomaly map 上采样方式、image-level aggregation protocol，以及每个数据集是否使用相同超参数。当前中文稿只描述方法核心，不应把这些实现项视为已经完整写入。
...
表 4 汇总了从 `paper_iconip/main.tex` 提取的代表性方法结果，并把当前 WPTA 结果放入同一候选表中。该表只用于判断 WPTA 在相近 OpenCLIP ViT-L/14 工业协议下的大致位置，不能单独支撑 SOTA claim，因为外部表使用 auxiliary-target ZSAD 协议，而 WPTA 草稿强调 training-free test-time adaptation。正式投稿时，如果无法证明协议完全一致，应把该表放入附录或删除。
...
在该候选表中，WPTA 在 MVTec/VisA 的 P-AUPRO 上具有较强位置，并在多数图像级指标上也具有竞争力。但由于协议尚未完全核验，本文将主 claim 限定为“在固定 AnomalyCLIP baseline 协议下稳定提升”，而不是“达到 SOTA”。

### 5.5 Appendix-style medical observation

ISIC/ISBI 上的附录式观察显示，WPTA 的 pixel AUROC / pixel AUPRO 从 AnomalyCLIP baseline 的 88.7 / 78.6 提升到 89.9 / 80.0。由于其它医学数据集当前没有完成评估，该结果只说明 reliability-guided calibration 在非工业场景中值得进一步验证，不作为本文主 claim，也不应写入摘要或贡献列表。
...
| WPTA has a preliminary positive observation beyond industrial data. | ISIC/ISBI improves from 88.7 / 78.6 to 89.9 / 80.0, but other medical datasets are blocked. | Appendix-only observation |
