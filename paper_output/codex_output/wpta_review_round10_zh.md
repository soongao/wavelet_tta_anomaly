...
2. 已移除用户指定删减项。扫描 v1.6 未发现相关中文或英文数据口径。
3. 已把 Figure 1/2 的资产路径描述改成正文中的图意说明，不再暴露文件路径或生成过程。
4. 已把五数据集结果与 Ours 机制消融拆开表述，避免把 MPDD/BTAD 的系统收益错误归因到 wavelet reliability。
...
| 2 | 五数据集主表的系统级 Ours 设置使用数据集特定配置。虽然 Table 4 已说明边界，但 Table 1 仍容易被读者当成统一方法在五数据集上的主结果。 | MAJOR | 将 Table 1 标题或 Method 名改成 `Dataset-specific calibrated inference system`，并在表注第一句说明不是同一模块组合。 |
| 3 | Ours 机制只在 MVTec/VisA 有受控消融。若摘要同时出现五数据集平均提升，读者可能期待 Ours 在五个数据集都有因果验证。 | MAJOR | 摘要中保留两个结论的拆分，或进一步减少五数据集平均提升在摘要中的权重。最好补 MPDD/BTAD/DTD-Synthetic 的同配置受控消融。 |
...
|---|---|---|---|
| Ours uses wavelet cues as evidence reliability rather than final-map scoring. | keep | Method 3.3/3.4, Table 2/3 direct fusion negative control | 机制清晰。 |
| Ours mechanism is validated on MVTec/VisA controlled ablations. | keep | Table 2/3 | 范围必须保持 MVTec/VisA。 |
...
| Ours mechanism is validated across all five datasets. | remove | Table 4 contradicts broad claim | v1.6 已避免。 |
