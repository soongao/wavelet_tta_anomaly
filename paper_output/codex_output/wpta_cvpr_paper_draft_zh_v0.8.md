...
当前稿件仍有四个投稿前限制。第一，`outputs/wpta_references_v0.1.bib` 已给出 BibTeX 草案，但部分数据集引用仍来自本地 `.bib`，需要最终 canonical metadata 核验后才能作为投稿版参考文献。第二，Figure 1 已有 MVTec/VisA 真实资产候选，但仍需要排版为投稿级图；Figure 3 的跨数据集 qualitative visualization 尚未生成，不能用手工热图替代。第三，外部方法比较尚未完成协议核验，不能用于主文强比较或 SOTA claim。第四，当前实现细节虽然已经从配置文件提取，但英文投稿版仍需要逐项核对 backbone、input size、cache generation、upsampling、smoothing 和 image-score aggregation。
...
- [ ] 基于 `outputs/wpta_figure_asset_ledger_v0.1.md` 中的真实 MVTec/VisA 机制图生成投稿级 Figure 1。
- [ ] 生成真实 Figure 3，覆盖至少四个工业数据集，并正确区分 wavelet reliability 与 dataset-specific calibration evidence。
