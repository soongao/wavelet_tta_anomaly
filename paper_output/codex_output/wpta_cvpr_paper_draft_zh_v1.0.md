...
本稿用于内部技术审阅，目标写作标准对齐 CVPR/ICCV 类计算机视觉顶会。中文仅用于便于审阅，不降低证据、引用、图表和实验要求。本文当前不声称外部最强方法地位，不使用未完成实验补数，不把不同数据集的 final setting 统一归因为同一机制。
...
目标：基于已生成的真实 Figure 1 资产重绘投稿级 Figure 1，而不是重新生成或手工修改热图。输入资产包括 `outputs/figures/figure1_motivated_example_mvtec_cable.png`、`outputs/figures/figure1_panels/` 中 7 个 panel，以及溯源文件 `outputs/figure1_caption_and_provenance_v0.1.md`。保持所有热图、reliability map、selected evidence 和 final map 的像素内容不变，只重绘矢量文字、箭头、panel 标题、图例和一行机制说明。横向 7-panel 布局：Input、GT target、Fixed prototype map、Direct wavelet cue、Boundary-aware reliability、Selected evidence、WPTA final map。异常 evidence 用橙色边框，normal evidence 用蓝色边框；字体缩放后不小于 8pt；白底；导出 PDF/SVG。图注第一句必须是：小波线索适合作为原型适配证据的可靠性监督，而不是直接替代语义异常分数。不得声称该单例代表所有数据集。
[/FIGURE_PROMPT]
