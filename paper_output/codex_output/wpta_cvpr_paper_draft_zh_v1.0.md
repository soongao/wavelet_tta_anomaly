...
本稿用于内部技术审阅，目标写作标准对齐 CVPR/ICCV 类计算机视觉顶会。中文仅用于便于审阅，不降低证据、引用、图表和实验要求。本文当前不声称外部最强方法地位，不使用未完成实验补数，不把不同数据集的 system-level setting 统一归因为同一机制。

当前数据证据来自 `/Users/bytedance/code/AnomalyCLIP/paper/result_record/result_table.csv` 中标记为 `current` 的工业结果。表格包见 `outputs/wpta_generated_tables_v0.6.md` 和 `outputs/wpta_generated_tables_latex_v0.6.tex`；Figure 1 真实机制图资产见 `outputs/figures/figure1_motivated_example_mvtec_cable.png`、`outputs/figures/figure1_motivated_example_mvtec_cable.pdf` 和 `outputs/figure1_caption_and_provenance_v0.1.md`。引用核验状态见 `outputs/wpta_citation_ledger_v0.1.md`，BibTeX 草案见 `outputs/wpta_references_v0.1.bib`。本文已移除正文中的未核验引用占位；仍保留必要的 `FIGURE_PROMPT` 和 `TABLE_DATA_PROMPT` 作为后续图表生成或数据补齐指令，它们不是最终投稿正文。
...
该图来自真实模型输出 strip，当前脚本只做裁切、标签和排版；由于源 panel 是 raster PNG，当前 PDF 是 raster 内容封装的排版候选。它可以支撑单例机制解释，不能替代跨数据集 qualitative evidence。
...
当前可用的附录表包括：Appendix Table A1 报告 MVTec/VisA 系统校准栈消融，Appendix Table A2 报告 ISIC/ISBI 医学补充结果，Appendix Table B1a/B1b 分别报告五数据集外部方法 pixel-level 与 image-level 协议参考比较。B1a/B1b 暂不加粗最佳值，也不用于外部最佳方法结论。
...
| Current system reaches external best-method status. | External comparison protocol is not fully verified. | Not supported; do not claim |
