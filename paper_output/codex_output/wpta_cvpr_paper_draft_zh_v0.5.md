...
本文采用两层实验叙事来避免把系统级优化误写成单一机制收益。第一层是 MVTec/VisA 的受控消融，用于验证 Ours 机制：semantic-only prototype adaptation 已显著强于固定原型；wavelet-guided evidence weighting 在 semantic-only 基础上继续提升；conservative update 进一步改善当前 Ours 方法。第二层是五个工业异常检测基准上的系统级 Ours 设置结果，包括 MVTec、VisA、MPDD、BTAD 和 DTD-Synthetic；与 AnomalyCLIP baseline 相比，系统级 Ours 设置的平均结果从 94.9 / 82.8 / 86.2 / 89.4 提升到 95.9 / 87.4 / 89.5 / 92.2。最显著的平均提升来自 pixel AUPRO，达到 +4.5，说明系统级 Ours 设置主要改善异常区域定位质量。
...
3. 本文在冻结 CLIP 参数、不使用目标训练数据、不进行反向传播的条件下进行 conservative prototype calibration，并通过 MVTec/VisA 受控消融验证 Ours 机制。
4. 本文报告五个工业基准上的系统级 Ours 设置结果，并用 system-level-setting 配置表明确区分系统级提升和 Ours 机制证据，避免把不同数据集的工程配置误归因为同一机制。
...
受控 Ours 消融在 MVTec 和 VisA 上使用相同的缓存特征与评价设置。我们使用 AnomalyCLIP 缓存的 patch features，并聚合 feature map layers 1/2/3；layer fusion 使用 sum，layer temperature 为 1.0，Gaussian smoothing 参数 `sigma=5`，AUPRO 使用 200 个阈值步。所有指标均按 image-pixel-level protocol 计算。
...
### 4.3 Baselines, system-level Ours settings, and variants

主结果以 AnomalyCLIP baseline 作为固定原型基线，并比较每个数据集的 system-level Ours setting。受控消融在 MVTec 和 VisA 上报告四个 variants：direct wavelet fusion / no adaptation、semantic prototype adaptation、Ours w/o conservative update，以及 Ours controlled ablation setting。由于五数据集 system-level Ours settings 与受控消融设置不完全相同，本文在表格和文字中显式区分两者：Table 1 证明系统级 Ours 设置有效性，Table 2 和 Table 3 证明 Ours 机制。

## 5. 结果与分析

### 5.1 Main results on five industrial benchmarks

表 1 展示五个工业数据集上的主结果。系统级 Ours 设置在所有数据集上均超过 AnomalyCLIP baseline，并且平均四个指标全部提升。该表不单独证明所有提升都来自 Ours；各数据集 system-level setting 的启用模块见 Table 4。
...
平均结果显示，系统级 Ours 设置的最大收益来自 P-AUPRO，平均提升 +4.5。该趋势说明当前系统主要改善局部异常区域覆盖，而不是只提升图像级分类分数。BTAD 和 MPDD 上的 image-level gains 也较明显，分别在 I-AUROC/I-AP 上提升 +4.8/+3.9 和 +4.1/+5.8，说明定位质量改善没有牺牲图像级检测。由于 MPDD/BTAD final commands 主要启用 multi-crop fusion 和 pixel-to-image fusion，本段只作为系统级结果解释；Ours 的机制解释见下一节受控消融。
...
Table 4 记录五数据集主结果中各 final run 的启用模块。该表的目的不是增加一个新结果，而是明确主结果表的证据边界。MVTec 和 VisA 的 system-level Ours setting 包含 wavelet reliability、TTA rectification、multi-crop fusion 和 pixel-to-image fusion；DTD-Synthetic 包含 wavelet reliability、multi-crop fusion 和 pixel-to-image fusion；MPDD 和 BTAD 当前 system-level Ours setting 没有启用 wavelet/TTA flags。因此，五数据集结果应写成 system-level Ours setting 的系统级提升，Ours 机制则由 MVTec/VisA 受控消融验证。
...
表 5 汇总了从 `paper_iconip/main.tex` 提取的代表性方法结果，并把当前 system-level Ours setting 结果放入同一候选表中。该表只用于判断当前系统在相近 OpenCLIP ViT-L/14 工业协议下的大致位置，不能单独支撑 SOTA claim，因为外部表使用 auxiliary-target ZSAD 协议，而当前稿件强调 training-free test-time adaptation。正式投稿时，如果无法证明协议完全一致，应把该表放入附录或删除。
...
在该候选表中，当前系统在 MVTec/VisA 的 P-AUPRO 上具有较强位置，并在多数图像级指标上也具有竞争力。但由于协议尚未完全核验，本文将主 claim 限定为“相对固定 AnomalyCLIP baseline 的系统级提升”和“MVTec/VisA 受控消融中的 Ours 机制有效”，而不是“达到 SOTA”。

### 5.6 Appendix-style medical observation

ISIC/ISBI 上的附录式观察显示，system-level Ours setting 的 pixel AUROC / pixel AUPRO 从 AnomalyCLIP baseline 的 88.7 / 78.6 提升到 89.9 / 80.0。由于其它医学数据集当前没有完成评估，该结果只说明当前校准系统在非工业场景中值得进一步验证，不作为本文主 claim，也不应写入摘要或贡献列表。
...
本文提出 Ours，用于 CLIP-based zero-shot anomaly detection。Ours 将 Haar 小波线索作为 patch evidence reliability 来监督测试时原型校准，而不是把频域响应直接融合到最终 anomaly map 中。受控 MVTec/VisA 消融说明，直接小波融合会损害定位表现，而 semantic-spectral evidence selection 和 conservative prototype calibration 能改善测试时原型适配。五个工业基准上的系统级 Ours 设置相对固定 AnomalyCLIP baseline 在平均 pixel AUPRO 上获得 +4.5 的提升，显示当前系统在异常区域定位上具有明确收益。后续投稿版本需要完成真实引用、协议核验和定性可视化，以支撑完整顶会证据链。

## 8. Claim-evidence map

| Claim | Evidence | Status |
|---|---|---|
| Ours (unnamed; system-level) improves AnomalyCLIP baseline on five industrial datasets. | Table 1, all five datasets improve over baseline; Table 4 records per-dataset system-level settings. | Supported |
...
- 证据：五数据集主结果支撑系统级 Ours 设置有效性，MVTec/VisA 受控消融支撑 Ours 机制；v0.5 已修正五数据集因果归因边界。
