...
这一比较范围是当前稿件最大的实验公平性限制之一。若投稿目标是 CVPR/ICCV，正式版本需要二选一：要么完成外部方法 protocol verification，并在同一 split、backbone、输入尺寸、后处理和 evaluation script 下报告可比主表；要么把论文定位为 AnomalyCLIP inference-stack calibration 与 WPTA 机制研究，并在摘要、引言和实验标题中明确不进行外部方法排名。当前 v1.5 采用后一种临时写法，因此所有相对结论只相对固定 AnomalyCLIP baseline 和受控 variants 成立。该定位仍不等价于投稿完成，因为顶会审稿通常会要求明确的强基线比较或充分论证为何该工作是机制研究而非排行榜论文。

## 5. 结果与分析

### 5.1 Main results on five industrial benchmarks

Table 1 展示五个工业数据集上的主结果，比较范围限定为固定 AnomalyCLIP baseline 与每个数据集的 final calibrated system。最终校准系统在所有数据集上均超过该固定 baseline，并且平均四个指标全部提升。该表不构成外部方法排名，也不单独证明所有提升都来自 WPTA；各数据集 final setting 的启用模块见 Table 4。
...
|---|---|---|---|---|
| Haar 小波作为 prototype adaptation 的 patch evidence reliability，而不是 final-map score | 3.3, 3.4, 6.1 | Table 2/3 中 direct wavelet fusion 低于 WPTA；boundary-aware reliability 优于 HF-only | 小波线索在受控 MVTec/VisA 设置中更适合作为 evidence reliability | 不能写成小波直接提升所有数据集 final map |
...
| Conservative prototype calibration | 3.5, 6.2 | Table 2/3 中 Full WPTA 相比 no-conservative variant 的 P-AUPRO 提升 | conservative update 在 MVTec/VisA 受控消融中进一步改善结果 | 当前默认 `alpha0=0.0`、`beta0=0.01`，实际 displacement 主要发生在 normal prototype |
| Final calibrated system 的系统级结果 | 4.3, 5.1, 5.4 | Table 1 五数据集相对固定 baseline 提升；Table 4 记录每个 final setting | final calibrated system 相对固定 AnomalyCLIP baseline 在五个工业基准上取得系统级提升 | 不能写成外部方法排名，也不能把 MPDD/BTAD 收益归因为 WPTA 机制 |
