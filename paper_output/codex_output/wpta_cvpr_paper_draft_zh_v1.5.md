...
当前受控实验的默认实现采用更保守的实例：`alpha0=0.0`、`beta0=0.01`。因此，abnormal evidence 主要用于判断当前图像是否有足够可靠的异常证据来打开更新门控，而实际 prototype displacement 主要发生在 normal prototype 上。本文保留上式作为通用形式，但所有结果解释均以该默认 conservative setting 为准。
...
所有实验使用 AnomalyCLIP checkpoint `9_12_4_multiscale/epoch_15.pth` 和 CLIP `ViT-L/14@336px` backbone。输入图像经 bicubic resize 与 center crop 处理到 `518 x 518`，并使用 OpenAI CLIP mean/std 归一化。缓存特征由 feature list `6/12/18/24` 产生；受控 Ours 消融在 MVTec 和 VisA 上读取同一批缓存 patch features，并聚合 feature map layers `1/2/3`。Layer fusion 使用 sum，layer temperature 为 1.0，Gaussian smoothing 参数 `sigma=5`，AUPRO 使用 200 个阈值步。所有指标均按 image-pixel-level protocol 计算。

Prototype adaptation 默认参数来自 `conf/run_prototype_ablation_experiments_conf.yaml`。Prototype softmax temperature 为 0.07，semantic sharpening `gamma=1.0`，wavelet evidence exponent `eta=1.0`，evidence top-k ratio 为 0.2。Conservative update 使用 `proto_alpha0=0.0`、`proto_beta0=0.01`、`proto_tau_a=0.15` 和 `proto_update_min_abnormal_confidence=0.06`，并在 confidence 不足时抑制更新。Boundary-aware wavelet reliability 使用 `proto_wavelet_mode=boundary_aware` 和 `proto_wavelet_mix=0.05`；HF-only 与 semantic-only variants 只改变 `proto_wavelet_mode`，no-conservative variant 只关闭 `proto_conservative_update`。Direct fusion 负对照不做 prototype adaptation，而是以 `direct_wavelet_fusion_weight=0.5` 直接融合 `S0` 与小波图。最终 prototype map 对每个所选层分别计算 calibrated abnormal probability，并通过 bilinear interpolation 上采样后按 sum 融合。
...
这一比较范围是当前稿件最大的实验公平性限制之一。若投稿目标是 CVPR/ICCV，正式版本需要二选一：要么完成外部方法 protocol verification，并在同一 split、backbone、输入尺寸、后处理和 evaluation script 下报告可比主表；要么把论文定位为 AnomalyCLIP inference-stack calibration 与 Ours 机制研究，并在摘要、引言和实验标题中明确不进行外部方法排名。当前 v1.5 采用后一种临时写法，因此所有相对结论只相对固定 AnomalyCLIP baseline 和受控 variants 成立。该定位仍不等价于投稿完成，因为顶会审稿通常会要求明确的强基线比较或充分论证为何该工作是机制研究而非排行榜论文。

## 5. 结果与分析

### 5.1 Main results on five industrial benchmarks

Table 1 展示五个工业数据集上的主结果，比较范围限定为固定 AnomalyCLIP baseline 与每个数据集的 system-level Ours setting。系统级 Ours 设置在所有数据集上均超过该固定 baseline，并且平均四个指标全部提升。该表不构成外部方法排名，也不单独证明所有提升都来自 Ours；各数据集 system-level setting 的启用模块见 Table 4。
...
### 8.1 Contribution-validation matrix

| Contribution | Method location | Validation evidence | Allowed claim | Boundary |
|---|---|---|---|---|
| Haar 小波作为 prototype adaptation 的 patch evidence reliability，而不是 final-map score | 3.3, 3.4, 6.1 | Table 2/3 中 direct wavelet fusion 低于 Ours；boundary-aware reliability 优于 HF-only | 小波线索在受控 MVTec/VisA 设置中更适合作为 evidence reliability | 不能写成小波直接提升所有数据集 final map |
| Semantic-spectral evidence selection | 3.4, 3.5 | Table 2 中 semantic prototype adaptation 与 wavelet-guided variants 的相对变化 | `S0` 与 `W` 共同筛选 evidence 有助于构造 visual anchors | 不能把 baseline 到任一增强行的完整差值全部归因于该模块 |
| Conservative prototype calibration | 3.5, 6.2 | Table 2/3 中 Ours 相比 no-conservative variant 的 P-AUPRO 提升 | conservative update 在 MVTec/VisA 受控消融中进一步改善结果 | 当前默认 `alpha0=0.0`、`beta0=0.01`，实际 displacement 主要发生在 normal prototype |
| Ours (unnamed; system-level) 的系统级结果 | 4.3, 5.1, 5.4 | Table 1 五数据集相对固定 baseline 提升；Table 4 记录每个 system-level setting | system-level Ours setting 相对固定 AnomalyCLIP baseline 在五个工业基准上取得系统级提升 | 不能写成外部方法排名，也不能把 MPDD/BTAD 收益归因为 Ours 机制 |
...
|---|---|---|
| Ours mechanism is validated across all five industrial datasets. | Table 4 shows MPDD/BTAD final runs do not enable wavelet/TTA flags, and DTD-Synthetic uses a dataset-specific system-level setting. | Ours mechanism is supported by controlled MVTec/VisA ablations; five-dataset results support the system-level Ours setting. |
...
- [x] 更新当前可用表格包到 `outputs/wpta_generated_tables_v1.0.md`、`outputs/wpta_generated_tables_latex_v1.0.tex` 和 `outputs/wpta_generated_tables_manifest_v1.0.md`，并在正文中替换旧版本引用。
- [x] 增加 contribution-validation matrix，使贡献、方法位置、验证表格和禁止外推边界一一对应。
...
- [ ] 将已核对的 implementation details 转写为匿名投稿版 appendix，并移除本地绝对路径；若保留缓存评估协议，补充 cache generation command 摘要。
...
请补齐医学异常检测补充表，只保留完成可比评估的数据集。候选数据集包括 ISIC/ISBI、ColonDB、ClinicDB、Kvasir、HeadCT、BrainMRI、Br35H。对每个数据集输出 baseline 与 system-level Ours setting 的 pixel-level 或 image-level 指标，并标明是否具备同一 evaluation script、同一 cache generation setting 和同一 metric implementation。输出 CSV 字段：`dataset, task_level, method, p_auroc, p_aupro, i_auroc, i_ap, cache_dir, result_log, eval_script, status, exclusion_reason`。缺失或不可比的数据集不得填数，只写 exclusion_reason。
[/TABLE_DATA_PROMPT]
...
请为 MVTec、VisA、MPDD、BTAD 和 DTD-Synthetic 生成类别级结果表。每个类别至少包含 baseline 与 system-level Ours setting 的 `p_auroc, p_aupro, i_auroc, i_ap`，并给出差值。输出 CSV 字段：`dataset, category, method, p_auroc, p_aupro, i_auroc, i_ap, delta_vs_baseline, result_log, status`。若某类别没有可追溯日志或评价协议不同，将 `status` 标为 `missing_or_incomparable`，不要补数。
[/TABLE_DATA_PROMPT]
