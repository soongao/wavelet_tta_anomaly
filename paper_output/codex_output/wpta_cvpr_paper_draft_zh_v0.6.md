...
本稿用于内部技术审阅，目标写作标准对齐 CVPR/ICCV 类计算机视觉顶会。中文仅用于便于审阅，不降低证据、引用、图表和实验要求。本文当前不声称 SOTA，不使用未完成实验补数，不把不同数据集的 system-level setting 统一归因为同一机制。`runtime` 和 `normal stability` 已从本文数据口径删除。

当前数据证据来自 `/Users/bytedance/code/AnomalyCLIP/paper/result_record/result_table.csv` 中标记为 `current` 的工业结果。表格包见 `outputs/wpta_generated_tables_v0.4.md` 和 `outputs/wpta_generated_tables_latex_v0.4.tex`。引用核验状态见 `outputs/wpta_citation_ledger_v0.1.md`。本文仍保留少量 `CITATION-PENDING` 和 `FIGURE_PROMPT`，这些是投稿前必须关闭的 gate。
...
4. 报告五个工业基准上的 system-level Ours setting 结果，并用 system-level-setting 配置审计表明确区分系统级提升和 Ours 机制证据。

[FIGURE_PROMPT:figure1_motivated_example]
目标：生成 Figure 1，用一个真实工业样例展示 fixed prototype mismatch、direct wavelet fusion failure 和 Ours 的区别。建议使用金属件边缘附近细划痕或 VisA 小缺陷样例。四栏横向布局：Input image；AnomalyCLIP initial map，标出真实缺陷低估与正常边界误激活；Direct wavelet fusion map，标出高频边界伪激活；Ours evidence and final map，显示 `S0`、`W = HF * (1 - LF_edge)`、selected abnormal/normal evidence patches、visual anchors 和 final map。所有热图必须来自真实模型输出；异常 evidence 用橙色边框，normal evidence 用蓝色边框；字体不小于 8pt；白底矢量风格。
[/FIGURE_PROMPT]
...
频域分析和小波变换长期用于纹理分析和表面缺陷建模，但直接把频域响应作为异常分数会受到正常结构边界和材质纹理的干扰。[CITATION-PENDING: wavelet texture inspection] 本文不把小波用作最终 anomaly score，而是把小波作为 patch evidence reliability。Table 2 和 Table 3 中 direct wavelet fusion 明显低于 Ours，说明小波线索的角色定位是本文方法成立的关键。
...
| Ours | 初始固定，测试时校准 | 是 | 是 | 用 `S0` 与 boundary-aware `W` 选择 visual anchors，并保守校准 prototypes。 |
...
**表 2. Ours 核心组件消融。每个结果单元格为 P-AUROC / P-AUPRO / I-AUROC / I-AP。**
...
目标：生成 qualitative visualization。每行一个真实 case，列为 Input image、GT mask、AnomalyCLIP baseline map、Boundary-aware wavelet reliability W、Selected evidence patches、Ours/final map。至少覆盖 MVTec、VisA、MPDD、BTAD、DTD-Synthetic 中 4 个数据集。Baseline 与 final map 使用相同色标，W 使用 viridis，abnormal evidence 用橙色边框，normal evidence 用蓝色边框。所有热图必须来自真实模型输出，不能手工涂色。图注第一句必须说明核心观察：Ours 的 final map 相比 baseline 更聚焦真实缺陷，同时降低结构边界伪激活。
[/FIGURE_PROMPT]
