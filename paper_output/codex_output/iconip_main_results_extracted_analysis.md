...
这和当前 Ours 草稿里的 `training-free / no backpropagation / no target training data` 叙事不完全相同。因此，下面结果可以作为“候选外部方法对比数据源”，但正式写入 Ours 论文前必须确认协议一致性。
...
## 4. 与当前 Ours 数据的直接相关子集

这里把 `main.tex` 的 MVTec AD / VisA 外部方法结果与当前 Ours `current` 结果放在一起，方便判断是否可生成外部对比表。
...
| MVTec | Ours current | 91.8 | **86.2** | **94.1** | **97.4** | current CSV |
...
| VisA | Ours current | **96.2** | **91.7** | 84.3 | **87.3** | current CSV |

## 5. 关键分析

### 5.1 这些数据确实提供了其他 method 的主结果

`main.tex` 中可提取的外部方法包括：CLIP、WinCLIP、VAND、CoOp、AdaCLIP、AnomalyCLIP、AA-CLIP，以及该源论文自己的 Ours (TAAP/INPC)。对于 Ours 当前关注的 MVTec 和 VisA，这些方法都有 image-level 与 pixel-level 主结果。

### 5.2 Ours 在 MVTec/VisA 上的相对位置较强

若暂时把 `main.tex` 的外部结果与当前 Ours 结果放在同一候选表中，Ours 的表现如下：

- MVTec：Ours 在 P-AUPRO、I-AUROC、I-AP 上最高；P-AUROC 为 91.8，仅比 `main.tex` 中 AA-CLIP 的 91.9 低 0.1。
- VisA：Ours 在 P-AUROC、P-AUPRO、I-AP 上最高；I-AUROC 为 84.3，低于 AdaCLIP 的 85.8 和 AA-CLIP 的 84.6。
- 最强叙事仍然是 AUPRO：Ours 在 MVTec 上 P-AUPRO 为 86.2，高于 Source Ours 的 85.5 和 AA-CLIP 的 84.6；在 VisA 上 P-AUPRO 为 91.7，高于 Source Ours 的 88.3 和 AnomalyCLIP 的 87.0。

### 5.3 当前 Ours baseline 与 `main.tex` 的 AnomalyCLIP baseline 不完全一致
...
- 当前 Ours 草稿强调 training-free、test-time、no backpropagation。
- 如果 Ours current 结果是在完全不同的 training-free 协议下跑出来的，直接和 `main.tex` 的 fine-tuned ZSAD 表比较会被审稿人质疑。

更稳妥写法：

> Compared with representative CLIP-based ZSAD results reported under a similar OpenCLIP ViT-L/14 industrial protocol, Ours is competitive on MVTec and VisA and achieves the strongest Pixel-AUPRO among the collected methods. We report this comparison as a protocol-aligned reference and keep the main claim on the fixed AnomalyCLIP baseline.

中文审稿版：

> 与 `main.tex` 中相近 OpenCLIP ViT-L/14 工业协议下的代表性 CLIP-based ZSAD 结果相比，Ours 在 MVTec 和 VisA 上具有竞争力，并在 Pixel-AUPRO 上达到收集方法中的最高值。但由于当前 Ours 的 training-free 协议与该表的 auxiliary-target fine-tuning 协议仍需核验，主 claim 应继续写成超过固定 AnomalyCLIP baseline。
...
- Ours numbers are from current CSV;
