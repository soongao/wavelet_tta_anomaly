...
当前数据证据来自 `/Users/bytedance/code/AnomalyCLIP/paper/result_record/result_table.csv` 中标记为 `current` 的工业结果。表格包见 `outputs/wpta_generated_tables_v0.7.md` 和 `outputs/wpta_generated_tables_latex_v0.7.tex`；Figure 1 真实机制图资产见 `outputs/figures/figure1_motivated_example_mvtec_cable.png`、`outputs/figures/figure1_motivated_example_mvtec_cable.pdf` 和 `outputs/figure1_caption_and_provenance_v0.1.md`。引用核验状态见 `outputs/wpta_citation_ledger_v0.1.md`，BibTeX 草案见 `outputs/wpta_references_v0.1.bib`。本文已移除正文中的未核验引用占位；后续图表生成或数据补齐指令统一放在第 10 节“非投稿正文生成指令”，它们不是最终投稿正文。
...
- PNG preview: `outputs/figures/figure1_motivated_example_mvtec_cable.png`
- PDF candidate: `outputs/figures/figure1_motivated_example_mvtec_cable.pdf`
...
该图来自真实模型输出 strip，当前脚本只做裁切、标签和排版；由于源 panel 是 raster PNG，当前 PDF 是 raster 内容封装的排版候选，不满足最终投稿的矢量排版 gate。它可以支撑单例机制解释，不能替代跨数据集 qualitative evidence。Figure 1 的矢量重绘指令见第 10 节。
...
Figure 2 应作为方法总览图放在本小节附近，但当前稿件只保留生成指令，尚未生成投稿级矢量资产。方法图生成指令见第 10 节。
...
这一比较范围是当前稿件最大的实验公平性限制之一。若投稿目标是 CVPR/ICCV，正式版本需要二选一：要么完成外部方法 protocol verification，并在同一 split、backbone、输入尺寸、后处理和 evaluation script 下报告可比主表；要么把论文定位为 AnomalyCLIP inference-stack calibration 与 Ours 机制研究，并在摘要、引言和实验标题中明确不进行外部方法排名。当前 v1.1 采用后一种临时写法，因此所有相对结论只相对固定 AnomalyCLIP baseline 和受控 variants 成立。
...
- [ ] 基于 `outputs/figure1_caption_and_provenance_v0.1.md` 和 `outputs/figures/figure1_panels/` 将 Figure 1 重绘为投稿级矢量排版。
...
[FIGURE_PROMPT:figure1_vector_redraw]
目标：基于已生成的真实 Figure 1 资产重绘投稿级 Figure 1，而不是重新生成或手工修改热图。输入资产包括 `outputs/figures/figure1_motivated_example_mvtec_cable.png`、`outputs/figures/figure1_panels/` 中 7 个 panel，以及溯源文件 `outputs/figure1_caption_and_provenance_v0.1.md`。保持所有热图、reliability map、selected evidence 和 final map 的像素内容不变，只重绘矢量文字、箭头、panel 标题、图例和一行机制说明。横向 7-panel 布局：Input、GT target、Fixed prototype map、Direct wavelet cue、Boundary-aware reliability、Selected evidence、Ours final map。异常 evidence 用橙色边框，normal evidence 用蓝色边框；字体缩放后不小于 8pt；白底；导出 PDF/SVG。图注第一句必须是：小波线索适合作为原型适配证据的可靠性监督，而不是直接替代语义异常分数。不得声称该单例代表所有数据集。
[/FIGURE_PROMPT]
