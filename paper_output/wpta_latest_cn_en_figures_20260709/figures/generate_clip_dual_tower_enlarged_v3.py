...
PNG_OUT = OUT_DIR / "figure4_clip_dual_tower_enlarged_v3.png"
DRAWIO_OUT = OUT_DIR / "figure4_clip_dual_tower_enlarged_v3.drawio"
AUDIT_OUT = OUT_DIR / "figure4_clip_dual_tower_enlarged_v3.audit.md"
...
def similarity_input_slot(img: Image.Image, x: int, y: int, mode: str, lane: str) -> None:
    draw = ImageDraw.Draw(img)
    if lane == "image":
        rr(img, (x, y, 300, 86), "#FFFFFF", COL["image"], radius=12, width=2)
    ...
    stroke = COL["line_dark"]
    fill = "#FFFFFF"
    label = "Fixed prototype set P"
    note = "p_N and p_A are unchanged"
    color = COL["muted"]
    if mode == "direct":
        stroke, fill, label, note, color = COL["red"], COL["red_pale"], "Drifted prototype set P'", "raw TTA contaminates p_N", COL["red"]
    elif mode == "wavelet":
        stroke, fill, label, note, color = COL["green"], COL["green_pale"], "Adapted prototype set P*", "gated update preserves p_A", COL["green"]

    rr(img, (x, y, 300, 86), fill, stroke, radius=12, width=2)
    text_box(draw, (x + 18, y + 8, 264, 16), label, F["small_b"], color)
    draw.ellipse((x + 64, y + 34, x + 110, y + 80), fill=COL["image_soft"] if mode != "direct" else COL["red_soft"], outline=stroke if mode != "fixed" else COL["image"], width=3)
    text_box(draw, (x + 64, y + 47, 46, 16), "p_N" if mode != "direct" else "p_N'", F["tiny_b"], color if mode != "fixed" else COL["image"])
...
def adaptation_branch(img: Image.Image, x: int, y: int, w: int, h: int, mode: str) -> None:
    draw = ImageDraw.Draw(img)
    if mode == "fixed":
        rr(img, (x + 80, y + 2, w - 160, h - 4), COL["gray_pale"], COL["line_dark"], radius=12, width=2)
        text_box(draw, (x + 102, y + 8, w - 204, 16), "No test-time feedback branch", F["small_b"], COL["muted"])
        draw.line((x + w // 2 - 22, y + 27, x + w // 2 + 22, y + 27), fill=COL["line_dark"], width=3)
        draw.line((x + w // 2 - 10, y + 17, x + w // 2 + 10, y + 39), fill=COL["red"], width=4)
        draw.line((x + w // 2 + 10, y + 17, x + w // 2 - 10, y + 39), fill=COL["red"], width=4)
        text_box(draw, (x + 134, y + 35, w - 268, 13), "zero-shot matching only", F["tiny"], COL["subtle"])
        return

    if mode == "direct":
        rr(img, (x, y, w, h), COL["red_pale"], COL["red"], radius=12, width=2)
        text_box(draw, (x + 14, y + 8, 132, 16), "Raw TTA branch", F["small_b"], COL["red"])
        for i, c in enumerate([COL["image_soft"], COL["red_soft"], COL["image_soft"], COL["red_soft"], COL["gray_soft"]]):
            draw.rounded_rectangle((x + 162 + i * 34, y + 14, x + 185 + i * 34, y + 36), radius=6, fill=c, outline=COL["line_dark"], width=1)
        arrow(draw, (x + 354, y + h // 2), (x + 444, y + h // 2), COL["red"], width=3, head=10)
        text_box(draw, (x + 448, y + 9, 120, 15), "update p_N", F["tiny_b"], COL["red"])
        text_box(draw, (x + 162, y + 38, 248, 13), "unfiltered tokens include abnormal evidence", F["tiny_b"], COL["red"])
        return

    rr(img, (x, y, w, h), "#FFFFFF", COL["green"], radius=12, width=2)
    text_box(draw, (x + 14, y + 8, 150, 16), "Wavelet feedback branch", F["small_b"], COL["green"])
    rr(img, (x + 178, y + 11, 58, 30), COL["teal_soft"], COL["teal"], radius=8, width=2)
    text_box(draw, (x + 178, y + 18, 58, 12), "DWT", F["tiny_b"], COL["teal"])
    arrow(draw, (x + 242, y + 26), (x + 290, y + 26), COL["green"], width=3, head=10)
    pill(img, (x + 300, y + 8, 82, 23), "LL pass", COL["green_soft"], COL["green"], COL["green"])
    pill(img, (x + 300, y + 34, 106, 23), "HF suppress", COL["orange_soft"], COL["orange"], COL["orange"])
    rr(img, (x + 430, y + 13, 74, 30), COL["green_pale"], COL["green"], radius=9, width=2)
    text_box(draw, (x + 430, y + 20, 74, 12), "gate", F["tiny_b"], COL["green"])
    text_box(draw, (x + 58, y + 37, 172, 13), "select reliable visual semantics", F["tiny"], COL["subtle"])
...
def dual_tower(img: Image.Image, row_y: int, mode: str, accent: str) -> None:
    ...
    text_box(draw, (x0 + 18, y0 + 10, 420, 20), "Enlarged CLIP dual tower with feedback path", F["small_b"], COL["ink"], align="left")
    ...
    if mode == "direct":
        arrow(draw, (x_feat + 120, y_img + 86), (adapt_lane[0] + 172, adapt_lane[1] + 14), COL["red"], width=4, dashed=True, head=12)
        arrow(draw, (adapt_lane[0] + 568, adapt_lane[1] + 29), (x_slot + 120, y_txt), COL["red"], width=4, dashed=True, head=12)
        text_box(draw, (adapt_lane[0] + 510, adapt_lane[1] + 34, 210, 16), "feedback to text prototype", F["tiny_b"], COL["red"])
    elif mode == "wavelet":
        arrow(draw, (x_feat + 120, y_img + 86), (adapt_lane[0] + 178, adapt_lane[1] + 26), COL["green"], width=4, dashed=True, head=12)
        arrow(draw, (adapt_lane[0] + 506, adapt_lane[1] + 28), (x_slot + 120, y_txt), COL["green"], width=4, dashed=True, head=12)
        text_box(draw, (adapt_lane[0] + 522, adapt_lane[1] + 34, 202, 16), "gated update to p_N", F["tiny_b"], COL["green"])
    else:
        draw.line((adapt_lane[0] + 372, adapt_lane[1] + 29, adapt_lane[0] + 420, adapt_lane[1] + 29), fill=COL["line_dark"], width=3)
...
def header(img: Image.Image) -> None:
    ...
    pill(img, (2426, 50, 64, 30), "v3", "#FFFFFF", COL["line_dark"], COL["subtle"])
...
class Drawio:
    ...
    def xml(self) -> str:
        return (
            ...
            '  <diagram id="clip-dual-tower-feedback-v3" name="CLIP dual tower feedback v3">\n'
...
def d_similarity_input_slot(d: Drawio, x: int, y: int, mode: str, lane: str) -> None:
    if lane == "image":
        d.rect(x, y, 300, 86, "Visual features to CLIP sim", "#FFFFFF", COL["image"], font_size=13, font_color=COL["image"], bold=True, radius=12, valign="top")
        for i in range(6):
            cx = x + 50 + i * 34
            cy = y + 48 + (8 if i % 2 else 0)
            d.ellipse(cx - 9, cy - 9, 18, 18, "", COL["image_soft"], COL["image"], stroke_width=2)
        d.text(x + 32, y + 68, 236, 14, "frozen image tower output V", 11, COL["subtle"], align="center")
        return
    ...
    if mode == "direct":
        ...
        stroke, fill, label, note, color = COL["green"], COL["green_pale"], "Adapted prototype set P*", "gated update preserves p_A", COL["green"]
    d.rect(x, y, 300, 86, label, fill, stroke, font_size=13, font_color=color, bold=True, radius=12, valign="top")
    d.ellipse(x + 64, y + 34, 46, 46, "p_N" if mode != "direct" else "p_N'", COL["image_soft"] if mode != "direct" else COL["red_soft"], stroke if mode != "fixed" else COL["image"], 11, color if mode != "fixed" else COL["image"], True, 3)
    d.ellipse(x + 186, y + 34, 46, 46, "p_A", COL["red_soft"], COL["red"], 11, COL["red"], True, 3)
    d.text(x + 36, y + 66, 228, 14, note, 11, color if mode != "fixed" else COL["subtle"], align="center")


def d_adaptation_branch(d: Drawio, x: int, y: int, w: int, h: int, mode: str) -> None:
    if mode == "fixed":
        d.rect(x + 80, y + 2, w - 160, h - 4, "No test-time feedback branch", COL["gray_pale"], COL["line_dark"], font_size=13, font_color=COL["muted"], bold=True, radius=12, valign="top")
        d.arrow(x + w // 2 - 22, y + 27, x + w // 2 + 22, y + 27, COL["line_dark"], 3, end=False)
        d.arrow(x + w // 2 - 10, y + 17, x + w // 2 + 10, y + 39, COL["red"], 4, end=False)
        d.arrow(x + w // 2 + 10, y + 17, x + w // 2 - 10, y + 39, COL["red"], 4, end=False)
        d.text(x + 134, y + 35, w - 268, 13, "zero-shot matching only", 11, COL["subtle"], align="center")
        return

    if mode == "direct":
        d.rect(x, y, w, h, "Raw TTA branch", COL["red_pale"], COL["red"], font_size=13, font_color=COL["red"], bold=True, radius=12, valign="top", align="left")
        for i, c in enumerate([COL["image_soft"], COL["red_soft"], COL["image_soft"], COL["red_soft"], COL["gray_soft"]]):
            d.rect(x + 162 + i * 34, y + 14, 23, 22, "", c, COL["line_dark"], radius=6, stroke_width=1)
        d.arrow(x + 354, y + h // 2, x + 444, y + h // 2, COL["red"], 3)
        d.text(x + 448, y + 9, 120, 15, "update p_N", 11, COL["red"], True, align="center")
        d.text(x + 162, y + 38, 248, 13, "unfiltered tokens include abnormal evidence", 11, COL["red"], True, align="center")
        return

    d.rect(x, y, w, h, "Wavelet feedback branch", "#FFFFFF", COL["green"], font_size=13, font_color=COL["green"], bold=True, radius=12, valign="top", align="left")
    d.rect(x + 178, y + 11, 58, 30, "DWT", COL["teal_soft"], COL["teal"], font_size=11, font_color=COL["teal"], bold=True, radius=8)
    d.arrow(x + 242, y + 26, x + 290, y + 26, COL["green"], 3)
    d_pill(d, x + 300, y + 8, 82, "LL pass", COL["green_soft"], COL["green"], COL["green"])
    d_pill(d, x + 300, y + 34, 106, "HF suppress", COL["orange_soft"], COL["orange"], COL["orange"])
    d.rect(x + 430, y + 13, 74, 30, "gate", COL["green_pale"], COL["green"], font_size=11, font_color=COL["green"], bold=True, radius=9)
    d.text(x + 58, y + 37, 172, 13, "select reliable visual semantics", 11, COL["subtle"], align="center")


def d_dual_tower(d: Drawio, row_y: int, mode: str, accent: str) -> None:
    x0, y0, w, h = 310, row_y + 92, 1500, 304
    d.rect(x0, y0, w, h, "", "#FFFFFF", COL["line"], radius=18)
    d.text(x0 + 18, y0 + 10, 420, 20, "Enlarged CLIP dual tower with feedback path", 13, COL["ink"], True)
    ...
    d.rect(x0 + 24, y0 + 42, w - 48, 92, "", COL["image_pale"], "#BFDBFE", radius=14)
    d.rect(x0 + 24, y0 + 204, w - 48, 90, "", COL["text_pale"], "#FCD34D", radius=14)
    ...
    d_similarity_input_slot(d, x_slot, y_img, mode, "image")
    ...
    d_similarity_input_slot(d, x_slot, y_txt, mode, "text")
    d_adaptation_branch(d, adapt_x, adapt_y, adapt_w, adapt_h, mode)
    ...
    if mode == "direct":
        d.arrow(x_feat + 120, y_img + 86, adapt_x + 172, adapt_y + 14, COL["red"], 3, dashed=True)
        d.arrow(adapt_x + 568, adapt_y + 29, x_slot + 120, y_txt, COL["red"], 3, dashed=True)
        d.text(adapt_x + 510, adapt_y + 34, 210, 16, "feedback to text prototype", 11, COL["red"], True, align="center")
    elif mode == "wavelet":
        d.arrow(x_feat + 120, y_img + 86, adapt_x + 178, adapt_y + 26, COL["green"], 3, dashed=True)
        d.arrow(adapt_x + 506, adapt_y + 28, x_slot + 120, y_txt, COL["green"], 3, dashed=True)
        d.text(adapt_x + 522, adapt_y + 34, 202, 16, "gated update to p_N", 11, COL["green"], True, align="center")
    else:
        d.arrow(adapt_x + 372, adapt_y + 29, adapt_x + 420, adapt_y + 29, COL["line_dark"], 3, end=False)
        d.arrow(adapt_x + 390, adapt_y + 16, adapt_x + 406, adapt_y + 42, COL["red"], 4, end=False)
        d.arrow(adapt_x + 406, adapt_y + 16, adapt_x + 390, adapt_y + 42, COL["red"], 4, end=False)
...
def write_drawio() -> None:
    ...
    d_pill(d, 2426, 50, 64, "v3", "#FFFFFF", COL["line_dark"], COL["subtle"])
    rows = [
        (
            120,
            "(a) Conventional CLIP-ZSAD",
